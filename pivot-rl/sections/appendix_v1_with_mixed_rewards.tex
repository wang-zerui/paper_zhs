...
In the single-domain experiments, OOD change denotes the score difference relative to the base model on benchmarks outside the training domain. In the multi-domain experiment, the four agentic benchmarks are all training domains, so the remaining suite serves as held-out retention evaluations rather than strict OOD tests.

For pivot selection, we use three progressively more selective subsets: \emph{random}, \emph{mixed-outcome} ($\mathcal{D}_{\mathrm{mixed}}$), and \emph{large-advantage-gap} ($\mathcal{D}_{\mathrm{adv}}$), the last requiring both nonzero reward variance and a large gap between the demonstrated action and the reference-policy mean.
...
We compare three pivot selection strategies on $\tau^2$-Bench:
\begin{itemize}
    \item random pivots: no filtering,
    \item mixed-outcome pivots: nonzero reward variance under the reference policy,
    \begin{align}
        ...
        \label{eq:dmixed}
    \end{align}
    \item large-advantage-gap pivots ($\mathcal{D}_{\mathrm{adv}}$): mixed-outcome pivots further filtered for a large gap between the demonstrated action's reward and the reference-policy mean, given by Eq.~\eqref{eq:dadv}.
\end{itemize}
Table~\ref{tab:tau_pivot_selection} shows a monotonic benefit from more selective filtering: random pivots already improve over SFT ($59.68$ vs.\ $58.44$), mixed-outcome pivots improve further ($61.38$), and large-advantage-gap pivots yield the best result ($63.81$).
...
Qwen + Large-Advantage-Gap Pivots
& 58.67 & 69.01 & 63.74 & 63.81 \\
\bottomrule
\end{tabular}
\caption{
Effect of pivot selection on $\tau^2$-Bench (best checkpoint over training).
Random turns improve over the base model, mixed-outcome pivots improve further, and large-advantage-gap pivots perform best.
}
