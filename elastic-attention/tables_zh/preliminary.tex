\begin{table*}[t]
\centering
\caption{不同模型稀疏率下的性能保留率。数值表示相对全注意力基线（Sparsity 0.0）的性能百分比，其中 100.00 表示与基线持平。行表示模型稀疏率，列表示评测任务。}
\label{tab:sparsity_performance_matrix}
\begin{small}
\begin{tabular}{l|cccccc}
\toprule
\textbf{稀疏率} & \textbf{Single-Doc QA} & \textbf{Multi-Hop QA} & \textbf{Summarization} & \textbf{Few-Shot} & \textbf{Synthetic} & \textbf{Code} \\
\midrule
