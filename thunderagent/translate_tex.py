...
PREFACE_REPLACEMENTS = {
    ...
    r"\def\sectionautorefname{Section}": r"\def\sectionautorefname{节}",
    r"\def\exampleautorefname{Example}": r"\def\exampleautorefname{示例}",
    r"\def\subsectionautorefname{Section}": r"\def\subsectionautorefname{节}",
    r"\def\appendixautorefname{Appendix}": r"\def\appendixautorefname{附录}",
    r"\title{ThunderAgent: A Simple, Fast and Program-Aware Agentic Inference System}":
    r"\title{ThunderAgent：一个简单、快速、程序感知的智能体推理系统}",
    r"\algrenewcommand\algorithmicrequire{\textbf{Input:}}":
    r"\algrenewcommand\algorithmicrequire{\textbf{输入：}}",
    r"\algrenewcommand\algorithmicensure{\textbf{Output:}}":
    r"\algrenewcommand\algorithmicensure{\textbf{输出：}}",
    r"\input{Sections/sec0_abstract}": r"\input{Sections/sec0_abstract_zh}",
    r"\input{Sections/sec1_intro}": r"\input{Sections/sec1_intro_zh}",
    r"\input{Sections/sec2_background}": r"\input{Sections/sec2_background_zh}",
    r"\input{Sections/sec3_challenges}": r"\input{Sections/sec3_challenges_zh}",
    r"\input{Sections/sec4_methods}": r"\input{Sections/sec4_methods_zh}",
    r"\input{Sections/sec5_experiments}": r"\input{Sections/sec5_experiments_zh}",
    r"\input{Sections/appendix/extended_related_work}": r"\input{Sections/appendix/extended_related_work_zh}",
    r"\section{Conclusion}": r"\section{结论}",
    r"We introduce \ouralg, a fast and simple agentic system built on a program-level abstraction that tracks metadata throughout the entire lifecycle of each agentic workflow. \ouralg leverages the program abstraction for runtime scheduling and resource management. Specifically, \ouralg dynamically schedules program execution across GPU nodes to mitigate KV cache thrashing and memory imbalance, while managing tool resources to prevent resource leakage. Experimental results showcase that \ouralg outperforms previous systems by \textbf{1.48--3.58}$\times$ for serving and \textbf{1.79--3.92}$\times$ for RL rollouts.":
    r"我们提出了 \ouralg，这是一个快速而简单的智能体系统，建立在程序级抽象之上，能够在每个智能体工作流的完整生命周期内跟踪元数据。\ouralg 利用这种程序抽象进行运行时调度和资源管理。具体而言，\ouralg 在 GPU 节点之间动态调度程序执行，以缓解 KV 缓存抖动和显存不均衡，同时管理工具资源以防止资源泄漏。实验结果表明，在服务场景中 \ouralg 相比已有系统提升 \textbf{1.48--3.58}$\times$，在 RL rollout 场景中提升 \textbf{1.79--3.92}$\times$。",
    r"\section{Acknowledgements}": r"\section{致谢}",
    r"We are grateful to Together.ai for making this work possible. ":
    r"我们感谢 Together.ai 使这项工作成为可能。 ",
    r"We thank Ben Athiwaratkun and Ce Zhang for assistance in developing the multi-backend scheduler. ":
    r"感谢 Ben Athiwaratkun 和 Ce Zhang 在开发多后端调度器方面提供的帮助。 ",
    r"We thank Wenyi Hong and Luke Huang for helpful feedback and discussions during this work. ":
    r"感谢 Wenyi Hong 和 Luke Huang 在本工作期间提供的有益反馈和讨论。 ",
    r"\footnotetext{ $^{*}$Equal contribution. Correspond to \href{hkang342@gatech.edu}{hkang342@gatech.edu}}":
    r"\footnotetext{ $^{*}$共同一作。联系邮箱：\href{hkang342@gatech.edu}{hkang342@gatech.edu}}",
}
...
RAW_COMMANDS = {
    ...
    "setcounter", "renewcommand", "resizebox", "rotatebox", "centering", "raggedright",
    "small", "footnotesize", "scriptsize", "tiny", "large", "Large", "LARGE", "clearpage",
    "maketitlesupplementary", "bottomrule", "addlinespace", "arraystretch",
    "tabcolsep", "linewidth", "columnwidth", "textwidth", "hsize", "baselineskip",
    "oddsidemargin", "evensidemargin", "topmargin", "marginparwidth", "textheight",
    "textsc", "author", "affil", "date", "newtoggle", "toggletrue", "togglefalse",
    "iftoggle", "newcolumntype", "DeclareMathOperator", "lstdefinestyle",
    "lstdefinelanguage", "lstset", "algrenewcommand", "algorithmicrequire",
    "algorithmicensure", "makeatletter", "makeatother", "footnotemark",
    "footnotetext", "par", "addlogo", "newpage", "null", "vskip", "hfill",
    "hline", "cline", "faGithub", "faRss", "faWikipediaW", "samethanks",
    "ShowNotes", "myauthornote", "stepcounter", "g", "ouralg", "name",
    "shortname", "regname", "guideimp", "cmark", "xmark", "ding", "xspace",
    "@title", "@maketitle", "@copyrightspace", "@myauthornotes", "if@ACM@anonymous",
}
